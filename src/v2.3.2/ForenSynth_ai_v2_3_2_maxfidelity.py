...
import argparse, csv, hashlib, html as _html, json, math, os, random, re, shutil, subprocess, sys, time
...
def parse_args() -> AppConfig:
    ...
    p.add_argument("--max-fidelity", action="store_true", help="Enable integrity+evidence mode and conservative final call")
    ...
    return AppConfig(
        ...
        stream=stream, integrity=integrity, run_tests=a.run_tests, max_fidelity=a.max_fidelity,
...
def _extract_script(det: Dict[str, Any]) -> str:
    try:
        return ((((det.get("document") or {}).get("data") or {}).get("Event") or {}).get("EventData") or {}).get("ScriptBlockText", "") or ""
    except Exception:
        return ""
...
def evidence_from_detections(dets: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    iocs = {
        "ipv4": set(), "domains": set(), "urls": set(),
        "emails": set(), "md5": set(), "sha1": set(), "sha256": set(),
        "paths": set(),
    }
    ...
    for d in dets:
        ...
        if r: rules[r] = rules.get(r, 0) + 1

        # event id
        eid = _extract_event_id(d)
        if eid: eids[eid] = eids.get(eid, 0) + 1
        ...
        if u: users[u] = users.get(u, 0) + 1

        # scan for IOCs in rule+script text
        blob = " ".join([r, _extract_script(d)])
        for m in RE_IPV4.findall(blob):   iocs["ipv4"].add(m)
        for m in RE_DOMAIN.findall(blob): iocs["domains"].add(m)
        for m in RE_URL.findall(blob):    iocs["urls"].add(m)
        for m in RE_EMAIL.findall(blob):  iocs["emails"].add(m)
        for m in RE_MD5.findall(blob):    iocs["md5"].add(m.lower())
        for m in RE_SHA1.findall(blob):   iocs["sha1"].add(m.lower())
        for m in RE_SHA256.findall(blob): iocs["sha256"].add(m.lower())
        for m in RE_PATH.findall(blob):   iocs["paths"].add(m)

    # sort & limit heavy lists for readability
    def topn(d: Dict[str,int], n=20): return sorted(d.items(), key=lambda x: x[1], reverse=True)[:n]
    def sortl(s: set, n=50): return sorted(list(s))[:n]
...
def select_best_micros(blocks: List[List[Dict[str, Any]]], micros: List[str], budget_tokens: int, extra_tokens: int = 0) -> List[str]:
...
def build_html(md_final: str, dets: List[Dict[str, Any]], cfg: AppConfig, outname: str,
               md_sha256: Optional[str] = None, html_sha256: Optional[str] = None,
               evd: Optional[Dict[str,Any]] = None) -> str:
    ...
    if evd:
        ...
        evidence_section = (
            ...
            + "".join(tables) + "".join(ioc_blocks) + "</div>"
        )
...
def main():
    ...
    ensure_chainsaw(); ensure_paths(cfg.rules, cfg.mapping)
    ...
    if cfg.two_pass:
        ...
        final_text = call_llm(client, cfg.final_model, SYSTEM_FINAL, final_user, cfg.temperature, cfg.llm_timeout, cfg.llm_retries, stream=False if cfg.max_fidelity else cfg.stream)
        ...
        md_body = call_llm(client, cfg.final_model, SYSTEM_FINAL, final_user, cfg.temperature, cfg.llm_timeout, cfg.llm_retries, stream=False if cfg.max_fidelity else cfg.stream)
