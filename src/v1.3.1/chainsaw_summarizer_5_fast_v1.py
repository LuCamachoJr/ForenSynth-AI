...
import argparse, json, math, os, re, shutil, subprocess, sys, time, random, threading, csv
...
from openai import APIConnectionError, APIError, BadRequestError, RateLimitError, APITimeoutError
...
def load_detections(json_path: Path, mode: str, max_items: int) -> List[Dict[str, Any]]:
    ...
    if mode == "json":
        try: dets = _normalize_to_list(_parse_json_text(text))
        ...
        try:
            ...
            dets = flat
    if max_items > 0: dets = dets[:max_items]
    return dets
...
def micro_parallel(client: OpenAI, blocks: List[List[Dict[str,Any]]], model: str,
                   temperature: float, timeout_s: int, retries: int,
                   include_script: bool, micro_truncate: int,
                   rpm: int, workers: int) -> Tuple[List[str], int, int]:
    enc = get_encoder()
    ...
    with Progress(SpinnerColumn(), TextColumn("[bold]Micro[/bold]"),
                  BarColumn(), TextColumn("chunk [progress.completed]/[progress.total]"),
...
# ───────────────────────── Two-pass flow ──────────────────────────────────
def guardrail_estimate_or_die(chunks: List[List[Dict[str,Any]]], system_prompt: str,
                              max_chunks: int, max_input_tokens: int,
                              *, no_script: bool, truncate_script: int) -> Tuple[int, List[int]]:
    ...
    if total>max_input_tokens:
        die(f"Estimated input tokens {total} exceed --max-input-tokens {max_input_tokens}. "
            "Tune chunk size / truncate options.")
    return total, per

def two_pass_summarize(client: OpenAI, detections: List[Dict[str,Any]], cfg: AppConfig) -> Tuple[str,int,int,Dict[str,Tuple[int,int]]]:
    enc = get_encoder()
    ...
    if cfg.fast and cfg.micro_workers>1:
        micro_sections, mi_in, mi_out = micro_parallel(
            ...
            cfg.micro_include_script, cfg.micro_truncate, cfg.rpm, cfg.micro_workers
        )
    else:
        micro_sections = []
        mi_in = mi_out = 0
        with Progress(SpinnerColumn(), TextColumn("[bold]Micro[/bold]"),
            ...
            for i,b in enumerate(blocks):
                ...
                mi_in += estimate_tokens(enc, DEFAULT_SYSTEM_PROMPT)+estimate_tokens(enc, user)
                content = call_llm(client, cfg.chunk_model, DEFAULT_SYSTEM_PROMPT, user, cfg.llm_temperature, cfg.llm_timeout, cfg.llm_max_retries)
                mi_out += estimate_tokens(enc, content)
    ...
    final_content = call_llm(client, cfg.final_model, DEFAULT_FINAL_SYSTEM, final_user,
                             cfg.llm_temperature, cfg.llm_timeout, cfg.llm_max_retries)
...
RE_DOM = re.compile(r"\b(?!(?:localhost|localdomain)\b)(?:[a-z0-9](?:[a-z0-9\-]{0,61}[a-z0-9])?\.)+[a-z]{2,24}\b", re.IGNORECASE)
...
def extract_iocs(dets: List[Dict[str,Any]]) -> Dict[str, List[str]]:
    ...
    for d in dets:
        ...
        for k in ("ScriptBlockText","CommandLine","ParentCommandLine","TargetFilename","Image","DestinationHostname","DestinationIp"):
            v = evd.get(k)
            if isinstance(v,str):
                scan(v)
    return {
...
def format_iocs_md(iocs: Dict[str,List[str]]) -> str:
    ...
    if not blocks: return "No IOCs were extracted from the provided detections."
    return "\n\n".join(blocks)
...
def write_reports(base_dir: Path, llm_md: str, make_pdf: bool, make_html: bool, css_path: Optional[Path]) -> Tuple[Optional[Path],Optional[Path]]:
    ...
    if make_html:
        ...
        pypandoc.convert_text(
            ...
            extra_args=["--standalone","--toc","--toc-depth=3", f"--css={css_out}",
                        "--metadata","title=DFIR Chainsaw Summary (LLM)"]
        )
        html_out = html_path
...
def append_csv_summary(csv_path: Path, *, date: str, detections: int, chunks: int, micro_model: str, final_model: str,
                       runtime_s: float, micro_s: float, merge_s: float, cost: float, in_tokens:int, out_tokens:int):
    csv_path.parent.mkdir(parents=True, exist_ok=True)
    new_file = not csv_path.exists()
    with open(csv_path, "a", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        if new_file:
            w.writerow(["date","detections","chunks","micro_model","final_model","runtime_s","micro_s","merge_s","cost_usd","in_tokens","out_tokens"])
        w.writerow([date, detections, chunks, micro_model, final_model, f"{runtime_s:.2f}", f"{micro_s:.2f}", f"{merge_s:.2f}",
                    f"{cost:.6f}", in_tokens, out_tokens])
...
def main() -> None:
    ...
    _ = guardrail_estimate_or_die(chunks, DEFAULT_SYSTEM_PROMPT, cfg.max_chunks, cfg.max_input_tokens,
    ...
    # CSV summary
    append_csv_summary(cfg.csv_summary, date=today, detections=len(detections), chunks=len(chunks),
                       micro_model=cfg.chunk_model, final_model=cfg.final_model,
                       runtime_s=(time.perf_counter()-t0), micro_s=t_micro, merge_s=t_merge,
                       cost=est_cost, in_tokens=in_tokens, out_tokens=out_tokens)
    ...
    console.print(f"[cyan]🧠 Tokens used (est): in+out={in_tokens+out_tokens} (in={in_tokens}, out={out_tokens})[/cyan]")
    for ln in cost_lines: console.print(f"[cyan]{ln}[/cyan]")
